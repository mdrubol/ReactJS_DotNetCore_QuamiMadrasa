...
const columns: TableColumn<Head>[] = [
  {
    name: "Id",
    selector: (row: Head) => row.id,
    sortable: true
  },
  {
    name: "Name",
    selector: (row: Head) => row.name,
    sortable: true,
  },
  {
    name: "Type",
    selector: (row: Head) => row.type,
    sortable: true,
  },
  {
    button: true,
    cell: (row, index, column, id) => (
      <>
    <Link to={"/admin-dashboard/Head/"+row.id}> <span className="bi bi-pencil-square"></span></Link>
      </>
    )
  } 
];
...
function HeadList() {
  ...
  useEffect(()=>{
    ...
    accountingService.getAllHeads().then(resp=>{
  ...
  const onRowClicked = (row: Head, e: React.MouseEvent<Element, MouseEvent>) => {
    console.log("row",row, e);
  }

  const onSelectedRowsChange = (selected: { allSelected: boolean; selectedCount: number; selectedRows: Head[]}) => {
  ...
  return (
      isLoading ? <Loader/> : DataGrid<Head>(params)
  );
}
